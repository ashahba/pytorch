        ...
        # NB: These can't be put in type_dispatch, they have to run later
            ...
            if value.node.has_hint():
                ...
                if isinstance(value, torch.SymBool):
                    ...
                    created_new_symint = new_symint is None
                    if created_new_symint:
                        ...
                        if src_shape_env is not None:
                            value_range = src_shape_env.bound_sympy(src_expr)
                        ...
                        shape_env._register_unbacked_symint_input(
                            new_symint,
                            source=source,
                    ...
                    if not isinstance(new_expr, sympy.Symbol):
                        raise AssertionError(f"{new_expr} is not a basic Symbol.")
                    if source is not None and not created_new_symint:
                        self.tx.output.shape_env._register_unbacked_symint_input(
                            new_symint, source=source
                        )
